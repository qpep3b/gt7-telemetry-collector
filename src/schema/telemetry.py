...
class TelemetryStat(BaseModel):
    ...
    def from_bytes(cls, ddata: bytes, prev_event: 'TelemetryStat | None') -> 'TelemetryStat | None':
        ...
        type_speed_FL = abs(3.6 * tyre_diameter_FL * struct.unpack('f', ddata[0xA4:0xA4 + 4])[0])
        type_speed_FR = abs(3.6 * tyre_diameter_FR * struct.unpack('f', ddata[0xA8:0xA8 + 4])[0])
        type_speed_RL = abs(3.6 * tyre_diameter_RL * struct.unpack('f', ddata[0xAC:0xAC + 4])[0])
        tyre_speed_RR = abs(3.6 * tyre_diameter_RR * struct.unpack('f', ddata[0xB0:0xB0 + 4])[0])
        ...
        current_position = struct.unpack('h', ddata[0x84:0x84 + 2])[0]  # current position
        total_positions = struct.unpack('h', ddata[0x86:0x86 + 2])[0]  # total positions
        ...
        rpm_rev_warning = struct.unpack('H', ddata[0x88:0x88 + 2])[0]  # rpm rev warning

        brake = struct.unpack('B', ddata[0x92:0x92 + 1])[0] / 2.55  # brake

        boost = struct.unpack('f', ddata[0x50:0x50 + 4])[0] - 1  # boost

        rpm_rev_limiter = struct.unpack('H', ddata[0x8A:0x8A + 2])[0]  # rpm rev limiter

        estimated_top_speed = struct.unpack('h', ddata[0x8C:0x8C + 2])[0]  # estimated top speed

        clutch = struct.unpack('f', ddata[0xF4:0xF4 + 4])[0]  # clutch
        clutch_engaged = struct.unpack('f', ddata[0xF8:0xF8 + 4])[0]  # clutch engaged
        rpm_after_clutch = struct.unpack('f', ddata[0xFC:0xFC + 4])[0]  # rpm after clutch
        ...
        ride_height = 1000 * struct.unpack('f', ddata[0x38:0x38 + 4])[0]  # ride height
        ...
        angular_velocity_x = struct.unpack('f', ddata[0x2C:0x2C + 4])[0]  # angular velocity X
        angular_velocity_y = struct.unpack('f', ddata[0x30:0x30 + 4])[0]  # angular velocity Y
        angular_velocity_z = struct.unpack('f', ddata[0x34:0x34 + 4])[0]  # angular velocity Z
